...
public class HazelcastPortableGenerator extends UtilityGeneration {
	public HazelcastPortableGenerator() {
		super("Generate Hazelcast Portable write and read", "Select fields for portable");
	}
	...
	private void generatePortableWrite(PsiClass psiClass, List<PsiField> fields) {
		StringBuilder builder = new StringBuilder("@Override\n");
		builder.append("public void writePortable(PortableWriter out) throws IOException { \n");

		final List<PsiField> remaining = writeNativePortableFields(psiClass, fields, builder);
	...
	private List<PsiField> writeNativePortableFields(PsiClass psiClass, List<PsiField> fields, StringBuilder builder) {
		...
		for (PsiField field : fields) {
			...
			if(deepType.equals(PsiType.BOOLEAN) || deepType.equals(PsiType.BYTE) || deepType.equals(PsiType.CHAR) || deepType.equals(PsiType.SHORT)
				...
				if (portableType.isAssignableFrom(tt)) {
					writeField(builder, field.getName(), "Portable", isArray);
				}
	...
	private void generatePortableRead(PsiClass psiClass, List<PsiField> fields) {
		StringBuilder builder = new StringBuilder("@Override\n");
